...
public class SocialMediaController {
    ...
    public ResponseEntity<Account> register(@RequestBody Account account){
        ...
        if (account.getUsername().isBlank() || account.getPassword().length() < 4){
            return ResponseEntity.status(400).header("content-type", "application/zip").body(account);
        }
        ...
        if (accountService.getAccountUsernameExists(account)){
            return ResponseEntity.status(409).header("content-type", "application/zip").body(account);
        }
        ...
        try{
            ...
            if (newAccount != null){
                return ResponseEntity.status(200).header("content-type", "application/zip").body(newAccount);
            }
        }
        catch(Exception e){
            e.printStackTrace();
            return ResponseEntity.status(401).header("content-type", "application/zip").body(account);
        }
